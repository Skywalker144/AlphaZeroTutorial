...
if node.state.is_terminal():
	value = game.get_winner(node.state) * node.to_play  # 转换成当前节点玩家的视角
else:
	value = expand(node)
# -------------------------

def expand(node, add_dirichlet=False):
	encoded_state = game.encode_state(node.state, node.to_play)  # 将棋局编码
	nn_logits, nn_value = NN(encoded_state)  # 神经网络输出策略和相对价值
	
	nn_logits = mask_policy(nn_logits)  # 屏蔽非法动作（已有的棋子以及例如五子棋的禁手）
	
	nn_policy = softmax(nn_logits)
	
	if add_dirichlet:
		nn_policy = mix_dirichlet(nn_policy)
	
	for action in range(len(nn_policy)):
		to_play = node.to_play
		if nn_policy[action] > 1e-6:  # 只展开合法动作的子节点
			child = Node(
...
def search(state, to_play):
	...
	expand(root, add_dirichlet=True)
	...
	for _ in range(num_simulations):
		...
		if game.is_terminal(node.state):
			value = game.get_winner(node.state) * node.to_play  # 转换成当前节点的视角
		else:
			value = expand(node)
