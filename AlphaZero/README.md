...
用 Python、NumPy 和 PyTorch 实现策略价值网络、MCTS、自我对弈与训练，支持井字棋和自由规则五子棋，无需编译 C++。算法原理见 [AlphaZero](../docs/AlphaZero.md)。
...
- 自我对弈默认使用并行对局与批量网络推理；设置 `parallel=False` 时使用逐局、逐节点的串行实现。
