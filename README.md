...
AlphaZero 路线提供井字棋和五子棋的 Python 实现，包含自我对弈、训练、模型保存与恢复、命令行对弈和测试。KataGo 路线在此基础上实现了部分搜索与训练技巧。MuZero 路线实现了表征、动力学、预测三个网络与隐状态搜索，并提供串行与批量推理并行两种自我对弈后端；三条路线目前都没有系统化的棋力评估结论。
